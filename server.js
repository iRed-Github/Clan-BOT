...
•  - Creators: iRed & Sestro
•  - Discord: https://dsc.gg/idk-development
╰───────────────────────────────────────────⋞╯
`);
  client.user
    .setActivity(
      `+help | made by iRed with love`,
      { type: "WATCHING" }
    )
    .then(console.log)
    .catch(console.error);
...
readdirSync("./commands/").forEach(dir => {
  ...
  for (let file of commands) {
    ...
    console.log(`
╭⋟───────────────────────────────────────────╮
•  - Loaded ${command.name} from ${command.category}
•  - Creators: iRed & Sestro
•  - Discord: https://dsc.gg/idk-development
╰───────────────────────────────────────────⋞╯
`);
...
for (let file of fs.readdirSync("./events/")) {
  if (file.endsWith(".js")) {
    ...
    console.log(
      `
╭⋟────────────────────────────────────────────────────────────────────────────────╮
•  - Loaded ${description.name}${description.version} from ${description.filename}
•  - Creators: iRed & Sestro
•  - Discord: https://dsc.gg/idk-development
╰─────────────────────────────────────────────────────────────────────────────────⋞╯
`
 ...
 if (command.permission) {
  ...
  if (command.owner && message.author.id != `${message.guild.ownerID}`) {
    const owmer = new MessageEmbed()
      .setColor("RED")
      .setDescription(
        "❌ These commands can only be used by owner"
      );
  ...
  if (xpNeeded < xp) {
    ...
    if (message.guild.id === message.guild.id) {
      ...
      const ran = new canvacord.Rank()
        ...
        .setBackground(
            "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQz8TueHUgVNroUgCgN5d35oo5Kg_LuFN6CHToIXL4pPSK-AL8FXgjEyBUcJlg97sMcrqg&usqp=CAU"
        );
      ran.build().then(data => {
  ...
  if (guild.channel.name == sender.name) {
    ...
    fetch(
      `https://api.affiliateplus.xyz/api/chatbot?message=${encodeURIComponent(
        guild.content
      )}&botname=${client.user.username}&ownername=iRed`
    )
      .then(res => res.json())
      .then(data => {
...
client.on("message", async message => {
  ...
  webhook = webhook.find(x => x.name === "NRN" + number);

  if (!webhook) {
    webhook = await message.channel.createWebhook(`NRN`, {
